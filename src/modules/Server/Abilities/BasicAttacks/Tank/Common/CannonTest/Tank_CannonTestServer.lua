...
return {
	onFire = {
		function(state: BasicAttackState)
			...
			-- ✅ 서버 독립 origin: HRP 위치 + 클라이언트가 보낸 direction (단위벡터 검증은 BasicAttackService에서 완료)
			local origin = CFrame.new(hrp.Position, hrp.Position + state.direction)

			-- ✅ 서버 독립 aimRatio: 클라이언트 params 대신 state.aimTime 직접 계산
			...
			ProjectileHit.fire(state.attacker, origin, {
